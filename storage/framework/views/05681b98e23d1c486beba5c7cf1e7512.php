...
<?php $__env->startSection('title', __('Payment')); ?>
...
<div class="max-w-lg mx-auto px-4 py-6">
    <div class="bg-gradient-to-b from-rose-50 via-white to-amber-50 rounded-[2.5rem] shadow-xl shadow-rose-200/40 border border-rose-100/50 p-5 md:p-10">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-800"><?php echo e(__('Payment')); ?></h1>
            <p class="text-gray-400 text-sm mt-1"><?php echo e(__('Ticket Number')); ?>: <strong class="text-rose-600"><?php echo e($appointment->ticket_number); ?></strong></p>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-rose-100/50 space-y-4 shadow-sm">
            <div class="flex justify-between items-center py-2">
                ...
                <span class="text-gray-700 font-medium"><?php echo e($appointment->display_services ?: '—'); ?></span>
            </div>
            <div class="flex justify-between items-center py-2 border-t border-gray-50">
                <span class="text-gray-500"><?php echo e(__('Total')); ?></span>
                <span class="text-2xl font-extrabold text-rose-700"><?php echo e(formatCurrency($appointment->total_price)); ?></span>
            </div>
        </div>

        <form action="<?php echo e(route('payment.process', $appointment)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($appointment->guest_token): ?><input type="hidden" name="token" value="<?php echo e($appointment->guest_token); ?>"><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <div class="mb-6">
                <label class="block text-gray-700 font-bold text-sm mb-3"><?php echo e(__('Choose Payment Method')); ?></label>
                <div class="grid grid-cols-3 gap-3" id="payMethodContainer">
                    <label class="pay-option rounded-2xl px-4 py-4 cursor-pointer transition-all duration-200 text-sm font-medium border-2 flex flex-col items-center justify-center gap-2 border-rose-300 bg-rose-50/70 text-rose-700 shadow-sm">
                        <input type="radio" name="method" value="<?php echo e(Payment::METHOD_KNET); ?>" class="pay-radio hidden" checked onchange="updatePayMethod()">
                    ...
                    <label class="pay-option rounded-2xl px-4 py-4 cursor-pointer transition-all duration-200 text-sm font-medium border-2 flex flex-col items-center justify-center gap-2 border-gray-100 bg-white text-gray-500 hover:border-amber-200 hover:text-amber-600">
                        <input type="radio" name="method" value="<?php echo e(Payment::METHOD_APPLE_PAY); ?>" class="pay-radio hidden" onchange="updatePayMethod()">
                    ...
                    <label class="pay-option rounded-2xl px-4 py-4 cursor-pointer transition-all duration-200 text-sm font-medium border-2 flex flex-col items-center justify-center gap-2 border-gray-100 bg-white text-gray-500 hover:border-amber-200 hover:text-amber-600">
                        <input type="radio" name="method" value="<?php echo e(Payment::METHOD_GOOGLE_PAY); ?>" class="pay-radio hidden" onchange="updatePayMethod()">
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/payment/pay.blade.php ENDPATH**/ ?>