...
<div class="max-w-lg mx-auto px-4 py-6">
    <div class="bg-gradient-to-b from-rose-50 via-white to-amber-50 rounded-[2.5rem] shadow-xl shadow-rose-200/40 border border-rose-100/50 p-5 md:p-10 text-center">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ __('Payment Cancelled') }}</h1>
        <p class="text-gray-500 mb-2">{{ __('Ticket Number') }}: <strong class="text-rose-600">{{ $appointment->ticket_number }}</strong></p>
        <p class="text-gray-400 text-sm mb-6">{{ __('No Amount Deducted') }}</p>

        <div class="flex flex-col gap-3">
            <a href="{{ route('track.result', $appointment) }}" class="inline-flex items-center justify-center gap-2 bg-gradient-to-l from-rose-500 to-rose-600 text-white py-3.5 px-6 rounded-2xl font-bold shadow-lg shadow-rose-200/40 hover:shadow-xl hover:from-rose-600 hover:to-rose-700 transition-all duration-200">
