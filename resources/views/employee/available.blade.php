...
@section('page-title', __('Available Bookings'))

@section('content')
<form method="GET" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6">
    @php $selectedDate = request('date', now()->format('Y-m-d')); @endphp
    <div class="flex flex-wrap items-center gap-3">
        <input type="date" name="date" value="{{ $selectedDate }}" class="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition outline-none">
        <div class="flex gap-2">
            <a href="{{ route('employee.available', ['date' => now()->format('Y-m-d')]) }}" class="px-4 py-2.5 rounded-xl text-sm font-medium transition {{ $selectedDate === now()->format('Y-m-d') ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">{{ __('Today') }}</a>
            <a href="{{ route('employee.available', ['date' => now()->addDay()->format('Y-m-d')]) }}" class="px-4 py-2.5 rounded-xl text-sm font-medium transition {{ $selectedDate === now()->addDay()->format('Y-m-d') ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">{{ __('Tomorrow') }}</a>
        </div>
        <button type="submit" class="bg-gray-100 text-gray-700 px-5 py-2.5 rounded-xl hover:bg-gray-200 text-sm font-medium transition">{{ __('Filter') }}</button>
        @if(request('date'))
            <a href="{{ route('employee.available') }}" class="bg-white text-gray-500 px-5 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-sm transition">{{ __('Clear Filter') }}</a>
        @endif
    </div>
</form>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
    ...
    <div class="flex items-stretch gap-2 md:gap-4 group">
        ...
        <div class="flex-1 bg-white border border-gray-100 rounded-xl md:rounded-2xl p-3 md:p-4 group-hover:shadow-md group-hover:border-blue-200 transition-all">
            <div class="flex items-center gap-2 mb-1">
                ...
                <span class="inline-flex items-center gap-1 bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-lg text-[10px] font-bold leading-none border border-yellow-200/50">{{ __('Pending') }}</span>
            </div>
            <h3 class="font-bold text-gray-800 text-sm md:text-base mb-2">{{ $apt->customer_name }}</h3>
            ...
            <div class="space-y-1.5 text-xs md:text-sm text-gray-600 mb-3 md:mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <span>{{ $apt->display_services ?: '—' }}</span>
                </div>
                @if($apt->notes)
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-gray-500 truncate">{{ $apt->notes }}</span>
                </div>
                @endif
            </div>

            <form method="POST" action="{{ route('employee.appointment.claim', $apt) }}">
                @csrf @method('PATCH')
                <button type="submit" class="w-full bg-blue-600 text-white px-4 py-2 md:py-2.5 rounded-xl hover:bg-blue-700 active:scale-[0.98] font-medium text-xs md:text-sm transition-all">{{ __('Claim Booking') }}</button>
            </form>
        </div>
    </div>
    @empty
    <div class="col-span-full bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
        ...
        <p class="text-gray-500 text-lg">{{ __('No Available Bookings') }}</p>
        <p class="text-gray-400 text-sm mt-1">{{ __('All Bookings Assigned Or Completed') }}</p>
    </div>
    @endforelse
</div>
