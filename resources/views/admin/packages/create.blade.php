...
@section('page-title', __('Add Package'))

@section('content')
<form method="POST" action="{{ route('admin.packages.store') }}" class="max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    @csrf

    <div class="bg-gradient-to-l from-amber-50 to-white px-6 py-4 border-b border-gray-100">
        <h2 class="text-lg font-bold text-gray-800">{{ __('Add New Package') }}</h2>
    </div>
    ...
    <div class="p-6 space-y-6">
        ...
        <div>
            ...
            <textarea name="description_ar" rows="3" class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">{{ old('description_ar') }}</textarea>
        </div>

        <div>
            <label class="block text-gray-700 mb-2 text-sm font-medium">{{ __('Description (english)') }}</label>
            <textarea name="description_en" rows="3" class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">{{ old('description_en') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" step="0.001" name="original_price" value="{{ old('original_price') }}"
                    class="w-full rounded-xl px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition">
                <p class="text-xs text-gray-400 mt-1">{{ __('Price Before Discount Display Only') }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <label class="flex items-center gap-2 rounded-xl border border-gray-200 p-3.5 cursor-pointer hover:border-amber-400 transition @error('service_ids') border-red-300 @enderror">
