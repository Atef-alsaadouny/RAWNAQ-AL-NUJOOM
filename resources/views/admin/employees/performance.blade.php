...
@section('page-title', __('Performance') . ': ' . $employee->name)
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
        ...
        <div class="text-2xl font-bold text-amber-600">{{ $stats['total_appointments'] }}</div>
        <div class="text-gray-500 text-sm mt-0.5">{{ __('Total') }}</div>
    </div>
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-gray-500 text-sm mt-0.5">{{ __('Completed') }}</div>
    </div>
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-gray-500 text-sm mt-0.5">{{ __('Cancelled') }}</div>
    </div>
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-2xl font-bold text-purple-600">{{ number_format($stats['avg_rating'], 1) }}</div>
        <div class="text-gray-500 text-sm mt-0.5">{{ __('Avg Rating') }} <span class="text-xs">({{ $stats['total_ratings'] }})</span></div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
    <div class="flex items-center gap-2 mb-4">
        ...
        <h2 class="text-lg font-bold text-gray-800">{{ __('Completion Rate') }}</h2>
    </div>
    @if($stats['total_appointments'] > 0)
    <div class="rounded-full bg-gray-100 h-3">
        <div class="bg-gradient-to-l from-emerald-400 to-emerald-500 h-3 rounded-full" style="width: {{ ($stats['completed'] / $stats['total_appointments']) * 100 }}%"></div>
    </div>
    <p class="mt-2 text-gray-500 text-sm">{{ number_format(($stats['completed'] / $stats['total_appointments']) * 100, 1) }}% {{ __('Completion Rate') }}</p>
    @else
