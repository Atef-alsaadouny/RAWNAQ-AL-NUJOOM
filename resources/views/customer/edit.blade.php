...
<div class="max-w-2xl mx-auto px-4 py-6">
    <div class="text-center mb-8">
        ...
        <p class="text-gray-500 mt-1.5">{{ __('Ticket number') }}: <strong class="text-rose-600">{{ $appointment->ticket_number }}</strong></p>
    </div>

    <div class="bg-white/90 backdrop-blur-sm rounded-[2.5rem] shadow-xl shadow-rose-200/40 border border-rose-100/50 p-5 md:p-10">
        <form method="POST" action="{{ route('customer.appointment.update', $appointment) }}" id="editForm">
            @csrf
            @method('PUT')
