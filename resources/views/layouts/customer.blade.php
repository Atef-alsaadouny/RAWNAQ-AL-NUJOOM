...
<head>
    ...
    <title>@yield('title', __('My Account').' - '.__('site_name'))</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <meta name="description" content="{{ __('Customer Dashboard Description') }}">
            ...
            <div class="flex justify-between h-16 items-center">
                <a href="{{ route('home') }}" class="flex items-center gap-1.5">
                    ...
                    <div class="hidden md:flex flex-col justify-center">
                        @if(app()->getLocale() === 'en')
                            <span class="text-rose-900 font-bold text-lg leading-tight">{{ config('app.name') }}</span>
                            <span class="text-gray-500 font-medium text-xs tracking-wider" dir="rtl">رونق النجوم</span>
                        @else
                            <span class="text-rose-900 font-bold text-lg leading-tight">{{ __('site_name') }}</span>
