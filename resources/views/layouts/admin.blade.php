...
<head>
    ...
    <title>@yield('title', __('Admin Control').' - '.__('site_name'))</title>
...
<div class="flex h-screen">
    ...
    <aside id="sidebar" class="fixed md:relative w-64 bg-white shadow-lg z-40 h-full transition-all duration-300 md:translate-x-0 flex flex-col">
        ...
        <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
            <a href="{{ route('admin.dashboard') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.dashboard') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            ...
            <a href="{{ route('admin.appointments.index') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.appointments.*') && !request()->routeIs('admin.appointments.cancelled') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            ...
            <a href="{{ route('admin.employees.index') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.employees.*') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            ...
            <a href="{{ route('admin.customers.index') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.customers.*') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('admin.customers.*') ? 'text-amber-500' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                <span>{{ __('Customers') }}</span>
            </a>
            <a href="{{ route('admin.services.index') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.services.*') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
            ...
            <a href="{{ route('admin.packages.index') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.packages.*') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('admin.packages.*') ? 'text-amber-500' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                <span>{{ __('Packages') }}</span>
            </a>
            <a href="{{ route('admin.schedule') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.schedule') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5 shrink-0 {{ request()->routeIs('admin.schedule') ? 'text-amber-500' : 'text-gray-400 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>{{ __('Schedules') }}</span>
            </a>
            <a href="{{ route('admin.reports') }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.reports') ? 'bg-amber-50 text-amber-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
    ...
    <div class="flex-1 overflow-auto">
        <header class="bg-white shadow-sm p-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button id="notifBellBtn" onclick="toggleNotifPanel()" class="relative text-gray-500 hover:text-amber-600 transition-colors" title="{{ __('New Bookings') }}">
        ...
        {{-- Real-time notification panel (appears on all pages) --}}
            ...
            <div id="notifList" class="overflow-y-auto" style="max-height: calc(100vh - 190px);">
                <div class="flex items-center justify-center py-12 text-gray-400">
                    <div class="text-center">
                        ...
                        <p class="text-sm">{{ __('Awaiting New Bookings') }}</p>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        item.innerHTML =
            '<div class="flex items-start gap-3">' +
                ...
                '<div class="flex-1 min-w-0">' +
                    ...
                    '</div>' +
                '</div>' +
                '<a href="#" class="shrink-0 text-amber-600 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors" data-notif="link">{{ __("view") }}</a>' +
            '</div>';
    ...
    function pollNotifications() {
        fetch('{{ route("admin.notifications.recent") }}?since=' + encodeURIComponent(notifSeenTime), {
