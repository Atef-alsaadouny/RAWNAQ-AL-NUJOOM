...
<x-guest-layout>
    <div class="text-center mb-7">
        <h1 class="text-2xl font-bold text-gray-800">{{ __('Create New Account') }}</h1>
        <p class="text-gray-500 mt-1.5">{{ __('Track Bookings Easily') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" id="registerForm">
        ...
        <div>
            <label for="name" class="{{ $labelClass }}">{{ __('Name') }} <span class="text-red-500">*</span></label>
            <div class="relative">
                <div class="{{ $iconClass }}">
                ...
                <input id="name" class="{{ $inputClass }} ps-10" type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="{{ __('Full Name') }}" />
            </div>
            <x-input-error :messages="$errors->get('name')" class="mt-1.5" />
        </div>

        <div class="mt-5">
            <label for="phone" class="{{ $labelClass }}">{{ __('Phone Number') }} <span class="text-red-500">*</span></label>
            <div class="relative">
                <div class="{{ $iconClass }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input id="phone" class="{{ $inputClass }} ps-10" type="text" name="phone" value="{{ old('phone') }}" required placeholder="6xxxxxxx" inputmode="numeric" />
            </div>
            <x-input-error :messages="$errors->get('phone')" class="mt-1.5" />
        </div>

        <div class="mt-5">
            <label for="email" class="{{ $labelClass }}">{{ __('Email') }} <span class="text-xs text-gray-400 font-normal">({{ __('Optional') }})</span></label>
            <div class="relative">
                <div class="{{ $iconClass }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input id="email" class="{{ $inputClass }} ps-10" type="email" name="email" value="{{ old('email') }}" placeholder="example@email.com" />
            </div>
        ...
        <div class="mt-5">
            ...
            <div class="relative">
                ...
                <input id="password" class="{{ $inputClass }} ps-10 pe-12" type="password" name="password" required autocomplete="new-password" placeholder="••••••••" />
        ...
        <div class="mt-5">
            <label for="password_confirmation" class="{{ $labelClass }}">{{ __('Confirm Password') }} <span class="text-red-500">*</span></label>
            <div class="relative">
                <div class="{{ $iconClass }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                ...
                <input id="password_confirmation" class="{{ $inputClass }} ps-10 pe-12" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="••••••••" />
                <button type="button" id="togglePasswordConfirm" class="absolute inset-y-0 end-0 flex items-center pe-3.5 text-gray-400 hover:text-gray-600 transition">
        ...
        <div class="mt-8">
            <button type="submit" id="registerButton" class="w-full inline-flex items-center justify-center px-6 py-3 bg-gradient-to-l from-rose-500 to-rose-600 border border-transparent rounded-xl font-semibold text-[15px] text-white shadow-md shadow-rose-200 hover:from-rose-600 hover:to-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-400 focus:ring-offset-2 transition-all duration-200">
                <span id="buttonText">{{ __('Register Now') }}</span>
        ...
        <div class="text-center mt-6">
            <p class="text-sm text-gray-500">
                ...
                <a href="{{ route('login') }}" class="text-rose-600 hover:text-rose-700 hover:underline font-medium">{{ __('Log In') }}</a>
            </p>
        </div>
    </form>
